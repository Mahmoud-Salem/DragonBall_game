...
//			System.out.println("==> " + randomAttack.getClass().getSimpleName() + ": " + randomAttack.getName());
			...
			try {
				attack(randomAttack);
				vv="Attack";
			} catch (NotEnoughKiException | AlreadyTransformedException e) {
				block();
				vv= "Block";
			}
		} else {
	...
	public void attack(Attack attack) throws NotEnoughKiException, AlreadyTransformedException {
		vv="Attack";
		try {
		...
		{
			ArrayList <Attack> f = getAssignedAttacks() ;
			while(true)
			{
				int l= (int) (Math.random()*f.size());
				if( f.get(l) instanceof SuperSaiyan);
				else break ;
			}
			attack.onUse(attacker, getDefender(),
					(attacker == me && foeBlocking) || (attacker == foe && meBlocking));
